...
function systemPrompt(): string {
  return GOOSE_SYSTEM_PROMPT;
}
function mockReply(prompt: string): string {
  const preview =
    prompt.length > 80 ? `${prompt.slice(0, 80)}…` : prompt;
  return `(Mock) Placeholder for "${preview}"`;
}
...
function crisisHeuristic(userText: string): boolean {
  return /suicid|kill myself|self[- ]harm|hurt myself|end my life|want to die/i.test(
    userText
  ); 
} // these need to be updated/ enriched

function upsetHeuristic(userText: string): boolean {
...
function lightHeuristic(userText: string): boolean {
  const u = userText.trim();
  if (u.length > 100) return false;
  if (upsetHeuristic(u) || crisisHeuristic(u)) return false;
  return /\b(burger|yum|nice|great|happy|sunny|weather|lol|ate|lunch|sandwich|walk|today)\b/i.test(
    u
  );
}

function gooseReplyCharCap(userText: string): number {
  const u = userText.trim();
...
// 
function clipGooseReply(reply: string, userText: string): string {
  const cap = gooseReplyCharCap(userText);
  if (reply.length <= cap) return reply;

  // Search for a sentence boundary up to 30% past the cap so a sentence
  // that only slightly overruns is kept whole rather than chopped.
  const searchEnd = Math.min(reply.length, Math.floor(cap * 1.3));
  const searchIn = reply.slice(0, searchEnd);

  // Walk backwards from searchEnd to find the last sentence-ending punctuation.
  let lastEnd = -1;
  ...
  // If we found a sentence boundary covering at least half the cap, use it.
  if (lastEnd > Math.floor(cap * 0.5)) {
    return reply.slice(0, lastEnd + 1).trimEnd();
  }

  // Fallback: cut at a word boundary inside the cap, no ellipsis.
...
function completionMaxTokens(userText: string): number {
  const t = userText.trim();
  if (
    /suicid|kill myself|self[- ]harm|hurt myself|end my life|want to die/i.test(
      t
    )
  ) {
    return 260;
...
function ollamaUsesModelfilePersona(model: string): boolean {
  return /^goose(?::|$)/.test(model);
}

export async function askGoose(
  prompt: string,
  options: AskGooseOptions
): Promise<string> {
  ...
  if (provider === "ollama") {
    const model =
      ...
      "gemma3:latest";
    const msgs = ollamaUsesModelfilePersona(model)
      ? [{ role: "user", content: prompt }]
      : defaultMsgs;
...
export function parseLlmProvider(value: string | undefined | null): LlmProvider {
  if (value === "mock" || value === "ollama" || value === "groq") {
    return value;
  }
